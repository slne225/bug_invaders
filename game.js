...
function create() {
  ...
  this.physics.add.collider(gameState.enemies, gameState.bugRepellent, (bug, repellent) => {
    ...
    gameState.scoreText = this.add.text(175, 482, `Bugs Left: ${numOfTotalEnemies()}`, { fontSize: '15px', fill: '#000000' });
  })
...
function update() {
    ...
    if (numOfTotalEnemies() === 0) {
      gameState.active = false;
      this.physics.pause();
      gameState.scoreText = this.add.text(200, 200, 'You Win!', { fontSize: '15px', fill: '#000000' });
      gameState.enemyVelocity = 1;
    } else {gameState.enemies.getChildren().forEach(bug => {
